...
void *thread_func(void *arg) {
	...
	printf("Creating socket at %d interface with %s file\n", cfg.ifindex, cfg.filename);
	...
	printf("BPF %s loaded at %d interface\n",cfg.progsec, cfg.ifindex);
	...
	if (xsk_socket == NULL) {
		fprintf(stderr, "ERROR: Can't setup AF_XDP socket in interface %d\"%s\"\n",
			cfg.ifindex,strerror(errno));
		exit(EXIT_FAILURE);
	}
	
	printf("Socket at %d interface succesfully created\n", cfg.ifindex);
...
int main(int argc, char **argv)
{
	...
	struct config left = cfg; // Copy data from cfg to left
	left.ifindex = 3; // Modify ifindex in left

	struct config right = cfg; // Copy data from cfg to right
	right.ifindex = 4; // Modify ifindex in right

	/*pthread_t left_thread;
    	if (pthread_create(&left_thread, NULL, thread_func, (void *) &left) != 0) {
      		perror("Failed to create left_thread");
   		exit(EXIT_FAILURE);
    	}*/
	
  	pthread_t right_thread;
 	if (pthread_create(&right_thread, NULL, thread_func, (void *) &right) != 0) {
        	perror("Failed to create right_thread");
        	exit(EXIT_FAILURE);
  	}
