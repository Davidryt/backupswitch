...
func forwardFrames3(packetQueues *PacketQueue, output *xdp.Socket, priority int, gate bool) (numBytes uint64, numFrames uint64) {
    if !gate || priority < 0 || priority >= len(packetQueues.Queues) {
        return 0, 0 // Exit if the gate is closed or priority is out of range
    }

    queue := packetQueues.Queues[priority] // Select the queue based on the priority index
    ...
    for _, desc := range queue {
        ...
        inFrame := GetFrameFromDesc(desc) // Replace with actual function to get frame data from descriptor
        if len(inFrame) == 0 {
            continue // Skip empty or invalid frames
        }
