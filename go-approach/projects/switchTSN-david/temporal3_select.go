...
func launchswitch(verbose bool, inLink netlink.Link, inLinkQueueID int, outLink netlink.Link, outLinkQueueID int, queues int) {
	...
	var activein, tempin, activeout, tempout unix.FdSet
	FDZero()

	go func() {
    	fdsIn := new(syscall.FdSet)
    	fdsOut := new(syscall.FdSet)
 		...
 		for {
        	...
        	n, err := syscall.Select(int(inXsk.FD())+1, fdsIn, nil, nil, &tv)
        	if err != nil {
        	...
        	if n > 0 && fdsIn.IsSet(int(inXsk.FD())) {
	...
	for {
    	...
    	// Check for read readiness
    	n, err := syscall.Select(int(outXsk.FD())+1, fdsOut, nil, nil, &tv)
    	if err != nil {
    		if err == syscall.EINTR {
        	    continue // Handle EINTR (interrupted system call)
        	}
    	...
    	if n > 0 && fdsOut.IsSet(int(outXsk.FD())) {
    	    numBytes, numFrames := forwardFrames4(outXsk, inXsk, packetQueues)
    	    numBytesTotal += numBytes
    	    numFramesTotal += numFrames
    	}
