...
func main() {
	...
	var outLinkName2 string
	var outLinkQueueID2 int
	var queues int
	var verbose bool
	...
	flag.StringVar(&outLinkName2, "outlink2", "", "Output network link name.")
	flag.IntVar(&outLinkQueueID2, "outlinkqueue2", 0, "The queue ID to attach to on output link.")
	...
	if inLinkName == "" || outLinkName == "" || outLinkName2 == "" || queues == 0 {
		flag.Usage()
		os.Exit(1)
	}
	...
	outLink2, err := netlink.LinkByName(outLinkName2)
	if err != nil {
		log.Fatalf("failed to fetch info about link %s: %v", outLinkName2, err)
	}
	...
	launchswitch(verbose, inLink, inLinkQueueID, outLink, outLinkQueueID, outLink2, outLinkQueueID2, queues)
}
...
func launchswitch(verbose bool, inLink netlink.Link, inLinkQueueID int, outLink netlink.Link, outLinkQueueID int, outLink2 netlink.Link, outLinkQueueID2 int, queues int) {
	...
	log.Printf("attaching XDP program for %s...", outLink2.Attrs().Name)
	outProg2, err := xdp.LoadProgram("ebpf.o", "xdp_redirect", "qidconf_map", "xsks_map")
	if err != nil {
		log.Fatalf("failed to load xdp program: %v\n", err)
	}

	if err := outProg2.Attach(outLink2.Attrs().Index); err != nil {
		log.Fatalf("failed to attach xdp program to interface: %v\n", err)
	}
	defer outProg2.Detach(outLink2.Attrs().Index)

	log.Printf("opening XDP socket for %s...", outLink2.Attrs().Name)
	outXsk2, err := xdp.NewSocket(outLink2.Attrs().Index, outLinkQueueID2, nil)
	if err != nil {
		log.Fatalf("failed to open XDP socket for link %s: %v", outLink2.Attrs().Name, err)
	}

	if err := outProg2.Register(outLinkQueueID2, outXsk2.FD()); err != nil {
		fmt.Printf("error: failed to register socket in BPF map: %v\n", err)
		return
	}
	defer outProg2.Unregister(outLinkQueueID2)
