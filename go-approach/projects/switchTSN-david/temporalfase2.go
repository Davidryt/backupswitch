...
func main() {
	...
	log.Printf("Restarting interface %s...", inLinkName)
	err = restartNetworkInterface(inLinkName)
	if err != nil {
		log.Fatalf("Error reiniciando la interfaz de red: %v", err)
	}
	log.Printf("Restarting interface %s...", outLinkName)
	err = restartNetworkInterface(outLinkName)
	if err != nil {
		log.Fatalf("Error reiniciando la interfaz de red: %v", err)
	}
